...
Artikelbeschreibung:
Zustand: Gebraucht
Anzahl: {safe_value(row.get('Anzahl der Einheiten'))} Reifen

Reifen:
Jahreszeit: {safe_value(row.get('Reifenspezifikation'))}
Hersteller: {safe_value(row.get('Hersteller'))}
Modell: {safe_value(row.get('Modell'))}
Reifengröße: {safe_value(row.get('Reifenbreite'))}/{safe_value(row.get('Reifenquerschnitt'))} R{safe_value(row.get('Zoll'))} {safe_value(row.get('Tragfaehigkeitsindex'))}{safe_value(row.get('Geschwindigkeitsindex'))} {safe_value(row.get('Lastbereich'), '')}
Profiltiefe: {format_profile_depth(row.get('Profiltiefe'))}
DOT: {safe_value(row.get('DOT'))}
Schneeflocken-Symbol: {safe_value(row.get('Schneeflocken-Symbol'), 'Nein')}

Preis: 100€ 
Versand: 50€


(INTERN: {safe_value(row.get('CustomLabel'))})\n"""
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            ...
            for index, row in enumerate(csv_reader, 1):
                try:
                    ...
                    filename = f"output/tire_{safe_value(row.get('Reifenbreite', ''))}_{safe_value(row.get('Zoll', ''))}_{safe_manufacturer}_{index}.txt"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        csv_file = 'ebay_vorlage_reifen.csv'
